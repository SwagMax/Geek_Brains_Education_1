...
with open('bakery.csv','a', encoding='utf-8') as bakery_app:
    with open('bakery.csv', encoding='utf-8') as bakery:
        if len(argv) > 1 and [_ for _ in argv[1:] if _.replace('.','').isdigit()]:
            if len(argv) == 3:
                first, second = map(int, argv[1:])
                print(*(__ for _, __ in enumerate(bakery) if first - 1 <= _ < second), sep='')
            if ',' in argv[1] or '.' in argv[1]:
                if round(float(argv[1].replace(',', '.')), 3) <= 99999.999:
                    print(f"{round(float(argv[1].replace(',', '.')), 3):>010}", file=bakery_app)
                else:
                    print('Сумма до 99 999,999')
            else:
                print(*(__ for _, __ in enumerate(bakery) if _ >= int(argv[1]) - 1), sep='')
        else:
            print(bakery.read())