...
class Cell:
    ...
    def make_order(self, rows):
        return '\n'.join(["\N{grinning face}" * rows for i in range(self.nums // rows)]) + '\n' + "\N{winking face}" * (self.nums % rows)
    ...
    def __sub__(self, other):
        ...
        return Cell(self.nums - other.nums) if self.nums - other.nums > 0 else f"Вычитаем по модулю: {Cell(other.nums - self.nums)}"
